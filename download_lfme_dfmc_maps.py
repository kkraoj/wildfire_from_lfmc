...
def addGridmet(image):
  ...
  image = image.set("system:time_start",start)
  image = image.set("system:time_end", end)
  image = image.addBands(gridmet_.select("fm100"))
  image = image.addBands(gridmet_.select("fm1000"))
  return image
...
print(lfmc.size().getInfo())

n = lfmc.size().getInfo() # number of images to download
    
colList = lfmc.toList(n)

folder_name = "lfmc_dfmc_raw"  
scale = 4000
